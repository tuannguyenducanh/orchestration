...
import com.example.payment.model.OrderReplyEvent;
...
public class OrderReplyProducer {
	...
	public void sendMessage(OrderReplyEvent message) {
		log.info("Sending payment processed message: " + message);
		try {
			kafkaTemplate.send(ORDER_REPLY_TOPIC, objectMapper.writeValueAsString(message));
			log.info("Payment sent reply for order {} to topic {}", message.toString(), ORDER_REPLY_TOPIC);
